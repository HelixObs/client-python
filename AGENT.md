...
Any span event whose name starts with `helix.` is extracted by the gateway and written to the `entity_events` TimescaleDB table.

- `helix.error` — entity affected by an error (set by `instrument.error()`)
