...
- **Parent resolution**: known parents (same process) → OTel `Link`s. Unknown parents (cross-process) → `helix.parent.ids` comma-separated attribute; herald resolves server-side.
- **Entity operations**: `operate()` starts a new root trace (blank OTel context) for post-creation work on an existing entity. The herald detects `helix.entity.is_operation = "true"` and writes to `entity_operations` instead of `entities`. The entity's TraceStore entry is never overwritten by an operation. `entity_id` is optional — omit it to start the trace immediately and call `token.set_entity_id(id)` once the entity is known mid-operation. Spans that close without `entity_id` are forwarded as plain OTel traces (no DB write); a `WARNING` is logged.
- **Plain traces**: `tel.trace(name)` emits a normal OTel span with no entity semantics. Use it for infrastructure work (HTTP handlers, loops) where log correlation by trace ID is useful but no entity is being tracked. `tel.child_span()` calls inside a `tel.trace()` block automatically inherit the trace context. The herald forwards these spans unchanged.
...
## Span attribute names (herald contract)
